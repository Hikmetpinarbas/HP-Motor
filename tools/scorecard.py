...
def main():
    ...
    ap.add_argument("--phase", default=PHASE_DEFAULT)
    ap.add_argument("--label", default=LABEL_DEFAULT, help="phase label column name in phase file")
    ...
    if args.label not in ph.columns:
        raise SystemExit(f"Label column not found: {args.label}. Available: {ph.columns.tolist()}")

    # phase shares
    phase_counts = ph.pivot_table(index="team_name", columns=args.label, values="bin", aggfunc="count", fill_value=0)
    ...
    csv_out = os.path.join(args.outdir, "city_gs_scorecard.csv")
    ...
    ax.set_title(f"City–GS | Scorecard (phase={os.path.basename(args.phase)} | label={args.label})", pad=12)

    png_out = os.path.join(args.outdir, "city_gs_scorecard.png")
    plt.tight_layout()
    plt.savefig(png_out, dpi=150)
    plt.close(fig)

    print("[scorecard] core:", args.core)
    print("[scorecard] phase:", args.phase)
    print("[scorecard] label:", args.label)
